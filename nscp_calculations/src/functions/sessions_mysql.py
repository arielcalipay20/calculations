# src/sessions_mysql.py  (or whatever you named it)
import secrets, json
from datetime import datetime, timedelta
from typing import Optional

from src.functions.db_connection import db_conn  # <-- use the context manager
...
def create_session(user_id: int, meta: dict | None = None) -> str:
    ...
    with db_conn() as conn:
        ...
        try:
            cur.execute(
                "REPLACE INTO sessions(token, user_id, issued_at, expires_at, meta) "
                "VALUES (%s, %s, %s, %s, %s)",
